#Created on 2024/03/27 by using all possible piars of full siblings from each family
#Updated by using type=symm
#OpenMx
#updated on 2024/04/18 by modifying the definition for t1d_any: cases with only ICD250 and 
#without diagnostic code for T1D will not be included as T1D cases and by using debutar as a source of timing
...
AE_t1d6_any_onepair_nomod <- mxRun( AEmod , intervals=TRUE ) #0.84[95% CI: 0.76, 0.91]

#All fit attempts resulted in errors - check starting values or model specification
summary(AE_t1d6_any_onepair_nomod)
save(AE_t1d6_any_onepair_nomod, file="AE_t1d6_any_onepair_nomod.RData") #names to be changed
