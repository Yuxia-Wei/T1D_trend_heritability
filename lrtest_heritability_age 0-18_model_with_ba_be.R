#Supplementary Table 3: model fitting for likelihood ratio tests: T1D heritability at age 0-18_model 1(with ba and be parameters) based on all sibling pairs
#OpenMx
...
# Note that these CI's will be for birthyear==0. Because of the moderation due to ba and be the estimates will differ
#        for different values of birthyear.  
)
mxOption( NULL , 'Default optimizer' , 'CSOLNP' ) # May need to try another optimizer
# Fit the model
AE_t1d18_any <- mxRun( AEmod , intervals=TRUE )

summary(AE_t1d18_any)
save(AE_t1d18_any, file="AE_t1d18_any.RData") #names to be changed
