#Figure 3(B), Supplementary Table 5: bootstrapping process to calculate 95%CIs for heritability (age 0-6) in each birth year and heritability difference between birth  years based on model 1 (with ba and be parameter)
...
################################T1D at age 0-6 years#######################
### Source functions
source( 'W:/C6_Carlsson/Yuxia Wei/do/familial/240418_trend/source_bootfunction_t1d6.R' )
### Load data
...
saveRDS( aebootResults , 'W:/C6_Carlsson/Yuxia Wei/results/familial/trend_240418/bootrun_t1d6.Rds' )
aebootResults <- readRDS('W:/C6_Carlsson/Yuxia Wei/results/familial/trend_240418/bootrun_t1d6.Rds' )
...
replace h2_CI_t1d6_trend with h2dif_CI_t1d6_trend_SLSQP
and h2dif_CI_t1d6_trend with h2dif_CI_t1d6_trend_SLSQP

#trend of heritability
h2_CI_t1d6_trend=data.frame(rbind(quantile( aebootResults$h2_1982 , c(.025,0.5,.975) ),
...
h2_CI_t1d6_trend$Birth_year=seq(1982,2010,1)
names(h2_CI_t1d6_trend)=c("h2_lci","h2_median","h2_uci","Birth_year")
#Re-arrange column orders
h2_CI_t1d6_trend=subset(h2_CI_t1d6_trend,select=c(Birth_year,h2_median,h2_lci,h2_uci))

# trend of heritability difference
h2dif_CI_t1d6_trend=data.frame(rbind(quantile( aebootResults$h2dif_0085 , c(.025,0.5,.975) ),
...
h2dif_CI_t1d6_trend$Birth_year=c("2000 vs 1985", "2000 vs 1990", 
								"2000 vs 1995", "2010 vs 1985",
								"2010 vs 1990","2010 vs 1995")
names(h2dif_CI_t1d6_trend)=c("h2dif_lci","h2dif_median","h2dif_uci","Birth_year")
#Re-arrange column orders
h2dif_CI_t1d6_trend=subset(h2dif_CI_t1d6_trend,select=c(Birth_year,h2dif_median,h2dif_lci,h2dif_uci))
h2dif_CI_t1d6_trend$CI=paste("(",round(h2dif_CI_t1d6_trend$h2dif_lci,digit=3),", ",round(h2dif_CI_t1d6_trend$h2dif_uci,digit=3),")",sep="")
#trend of e2
e2_CI_t1d6_trend=data.frame(rbind(quantile( aebootResults$e2_1982 , c(.025,0.5,.975) ),
...
e2_CI_t1d6_trend$Birth_year=seq(1982,2010,1)
names(e2_CI_t1d6_trend)=c("e2_lci","e2_median","e2_uci","Birth_year")
#Re-arrange column orders
e2_CI_t1d6_trend=subset(e2_CI_t1d6_trend,select=c(Birth_year,e2_median,e2_lci,e2_uci))

library(openxlsx)
write.xlsx(h2_CI_t1d6_trend,file="h2_CI_t1d6_trend.xlsx",overwrite=TRUE)
write.xlsx(h2dif_CI_t1d6_trend,file="h2dif_CI_t1d6_trend.xlsx",overwrite=TRUE)
write.xlsx(e2_CI_t1d6_trend,file="e2_CI_t1d6_trend.xlsx",overwrite=TRUE)
getwd()
