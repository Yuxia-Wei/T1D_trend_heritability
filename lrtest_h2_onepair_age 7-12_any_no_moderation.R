#Supplementary Table 4: model fitting for likelihood ratio tests: T1D heritability at age 7-12_model 4 (without ba or be parameter) based on one sibling pair from each family
...
AE_t1d712_any_onepair_nomod <- mxRun( AEmod , intervals=TRUE ) #0.67[95% CI: 0.60, 0.74]

#All fit attempts resulted in errors - check starting values or model specification
summary(AE_t1d712_any_onepair_nomod)
save(AE_t1d712_any_onepair_nomod, file="AE_t1d712_any_onepair_nomod.RData") #names to be changed
