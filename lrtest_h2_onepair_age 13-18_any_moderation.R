#Supplementary Table 4: model fitting for likelihood ratio tests: T1D heritability at age 13-18_model 1(with ba and be parameters) based on one sibling pair from each family
#OpenMx
...
mxOption( NULL , 'Default optimizer' , 'CSOLNP' ) # not any warning message
# Fit the model
AE_t1d1318_any_onepair <- mxRun( AEmod , intervals=TRUE )

summary(AE_t1d1318_any_onepair)
save(AE_t1d1318_any_onepair, file="AE_t1d1318_any_onepair.RData") #"any" means any childhood-onset T1D (t1d18_any) regardless of conflicting tyeps of diagnosis
