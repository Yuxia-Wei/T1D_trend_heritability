#Figure 3 (A): source function for bootstrapping to calculate 95%CIs for heritability (age 0-6) in each birth year based on model 4 (without ba or be parameter)
...
aeFun <- function( datWideIn ){
  ...
  AE_t1d6_nomod <- mxRun( AEmod , intervals=TRUE )
  return( c(mxEval(h2,AE_t1d6_nomod),mxEval(e2,AE_t1d6_nomod)) )
}
